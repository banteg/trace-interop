...
**What this checks:** Assess this declared topic case. A failed top-level CREATE has no created-address match. Failed frames have an error string; an exceptional halt omits result or sets it to null. A REVERT frame keeps result {gasUsed, output}; a reverted CREATE has no address or code.
