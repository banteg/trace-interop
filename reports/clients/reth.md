...
| [Filter composition and mode](../decisions/H03.md)<br>Sender and recipient lists are combined with OR by default. | Differs<br>[Filter from only intersection](../cases/a/filter-from-only-intersection.md) | Differs<br>[Filter from only intersection](../cases/a/filter-from-only-intersection.md) | Use AND between lists so a second filter narrows the search; preserve union only as an explicit extension. Checked requirements: The portable filter profile rejects the mode extension as invalid params. Compare address bytes: OR within each list, AND across lists; missing/null/empty lists are unrestricted.<br>[Address filtering](https://github.com/paradigmxyz/reth/blob/03cb186c1d36eebbacc7bda08f36e25711d0804e/crates/rpc/rpc/src/trace.rs#L363) |
