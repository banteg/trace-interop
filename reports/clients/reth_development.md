...
| [Filter composition and mode](../decisions/H03.md)<br>Sender and recipient lists are combined with OR by default. | Differs<br>[Filter from only intersection](../cases/a/filter-from-only-intersection.md) | Use AND between lists by default so a second filter narrows the search; preserve union as an explicit mode.<br>[Address filtering](https://github.com/paradigmxyz/reth/blob/03cb186c1d36eebbacc7bda08f36e25711d0804e/crates/rpc/rpc/src/trace.rs#L363) |
...
| [Nonce-mismatch policy for signed simulation](../decisions/H13.md)<br>The high-nonce signed transfer is rejected instead of simulated. | Differs<br>[Raw nonce high](../cases/a/raw-nonce-high.md) | If the proposed nonce policy is adopted, permit nonce-mismatch simulation. This is a compatibility decision, not evidence of a nonce-rewriting bug; other validation checks remain separate. Checked requirements: Proposed nonce policy: simulate this otherwise valid transfer despite its signed nonce being above the state nonce.<br>[Signed transaction replay](https://github.com/paradigmxyz/reth/blob/03cb186c1d36eebbacc7bda08f36e25711d0804e/crates/rpc/rpc/src/trace.rs#L121) |
| [Invalid-parameter error codes](../decisions/H14.md)<br>An integer trace_get path returns null instead of invalid params. | Differs<br>[Filter from only intersection](../cases/a/filter-from-only-intersection.md) | Reject integer path entries with -32602; callers must convert output path integers to hex quantities. Checked requirements: Malformed input returns invalid params (-32602).<br>[Signed transaction replay](https://github.com/paradigmxyz/reth/blob/03cb186c1d36eebbacc7bda08f36e25711d0804e/crates/rpc/rpc/src/trace.rs#L121) · [Call simulation](https://github.com/paradigmxyz/reth/blob/03cb186c1d36eebbacc7bda08f36e25711d0804e/crates/rpc/rpc/src/trace.rs#L97) |
