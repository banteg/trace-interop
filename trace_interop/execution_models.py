...
def transactions(case):
    ...
    selected = params[2] if method in ['trace_call','trace_callMany'] and len(params)>2 else head
...
"""
    ...
    for b in context.get('_blocks', {}).values():
        ...
        for i,tx in enumerate(b['transactions']):
            ...
            if tx['to'] is None:
                exists.add(created_address(tx['sender'],tx['nonce']))
            elif tx['value']:
                exists.add(tx['to'])
...
def assess(case, observation, peers, topics):
    ...
    for i,(tx,block,index) in enumerate(models):
        ...
        if 'H17' in topics and isinstance(diff,dict):
            bad=[]
        ...
        if 'H19' in topics:
            code=tx['data'] if tx['to'] is None else codes.get(tx['to'])
            if isinstance(code,str) and code.startswith('0xef0100'):
                # Exactly one delegation hop. A target that delegates is not followed again.
                code=codes.get('0x'+code[8:],'0x')
            if code is not None:
                vm=e.get('vmTrace')
        ...
        if 'H16' not in topics or not isinstance(diff,dict):
            continue
        trace=e.get('trace')
        ...
        gas=quantity(mapping(receipt).get('gasUsed'))
